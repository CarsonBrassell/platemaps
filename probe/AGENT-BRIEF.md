...
## Searching: use Serper, not the WebSearch tool

```
node --env-file=.env.local scripts/serper.mjs "<name> <neighborhood> menu"
node --env-file=.env.local scripts/serper.mjs "<name> <street>" --maps     # website + address from Google Maps
```

This is real Google results through the Serper API, one compact line per hit,
and it is what finds the ordering-platform page (Toast, Square, ChowNow,
Clover, Menufy, popmenu…) that the restaurant's own site hides behind an
"Order Online" button. Search once with a specific query, then `--maps` if the
website is unknown. Each call costs one credit; do not spray queries. Confirm
identity (street number, city) on whatever it returns exactly as below.
...
**DoorDash truncates its own JSON-LD menu at about 91 raw items, and the cut is
...
**Two dish counts are where truncated captures pile up: 79 and 91 - a reason to
check, not a verdict.** Across 363 DoorDash menus already filed, the counts either side of 79 hold
...
the catalog twice. The router now runs this check itself: it compares its read against the menu
...
If your count lands on 79 or 91, treat the capture as
...
**A DoorDash or Uber Eats capture of 5-15 items is the front-page CAROUSEL, not
...
**The result file must contain EVERY dish you captured - never write a sample.**
