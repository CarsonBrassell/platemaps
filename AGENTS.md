...
- **Votes are thumbs**, not arrows: `ThumbsUpIcon`/`ThumbsDownIcon`, outline at rest, filled for the direction you pressed, one size in both states. Every vote surface uses the same pair — feed card, phone card, comments, map bubble, activity badge. The old `▲`/`△` text glyphs are retired (they changed size on click); the leaderboard's rank-change arrows are not votes and keep theirs.
