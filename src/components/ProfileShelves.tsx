...
export type ShelfPost = {
  ...
  upvoteCount: number;
  /** The other half of the pair the detail sheet's vote arrows read/write. */
  downvoteCount: number;
  /** This viewer's own vote — always the author's, since only your own plates
   *  reach the detail sheet from this component. Never both true. */
  upvotedByMe: boolean;
...
export function ProfileShelves({
  ...
  /**
   * A vote cast on the plate itself (not a comment), handed up for the same
   * reason: the sheet writes through `/api/posts/[id]/vote` but does not own
   * `posts`, so the array both the tile's meta line and the sheet's own vote
   * pair read has to be patched by whoever does.
   */
  onPostVoted?: (postId: string, patch: PostVotePatch) => void;
  /**
   * Delete a plate, handed up to whoever owns `posts` for the same reason the
   * comment callbacks are: this component does not own that array. The caller
