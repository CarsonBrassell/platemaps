...
### Two data sources that never mix

- **`src/data/*.ts`** — static TypeScript, generated by `scripts/fetch-*.mjs` from Yelp and real menu pages, committed and imported directly. Restaurants, dishes, regions, map comments. **Restaurants and dishes are not in Postgres**; pages import the array and look up by id.
- **Neon Postgres** — everything users produce: accounts, sessions, posts, upvotes, hearts, saves, comments, friendships, point events, aspect votes.

A restaurant page therefore joins two worlds: `restaurants.find(r => r.id === id)` from the static array, plus a DB read for that id. `restaurant_id` on `posts` is the seam.
...
- **Counts that never display:** friend/follower counts, anywhere. `getFriends` returns rows without a total on purpose. Also no busyness or wait-time data — invented wait copy shipped once and was deliberately removed (PRODUCT.md).
