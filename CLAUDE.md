...
**Restaurant identity is `sourceKey`, and `src/lib/sourceKey.ts` owns it.** The format is `<source>:<id>` — `yelp:tacos-el-gordo-chula-vista`, `osm:node/123`, `google:ChIJ…`. It replaced the bare Yelp alias when Yelp's free tier ended and restaurants started arriving from OpenStreetMap, which has no alias to parse. Three places must agree or the corpus grows duplicates: **both** serializers that rewrite `src/data/restaurants.ts` (`fetch-restaurants.mjs` and `blend-ratings.mjs` each write the array from scratch, so a field one of them doesn't know about vanishes on the next run), and the partial unique index `idx_restaurants_source_key`. `sourceKeyFor()` falls back to deriving the key from `yelpUrl`, so a row that loses the field degrades to the old behaviour instead of becoming unmatchable — treat its `null` as "refuse to merge", never as "no match found".

**`fetch-restaurants.mjs` merges, it does not replace.** A known business keeps its `id` and its position and a re-run cannot renumber the corpus — ids used to be array positions, which silently repointed every `posts.restaurant_id` on each run and made growing the corpus unsafe. A restaurant the search doesn't return is kept, never dropped; Yelp search is a ranked sample, not an enumeration. Flags: `--per-area`, `--min-reviews`, `--min-rating`, `--max-calls` (stops cleanly), `--no-hours`, `--hours-only` (search-free backfill of missing closing times, resumable).

**The feeds show a rolling window; the ratings show everything.** `FEED_WINDOW_DAYS` in `src/lib/feedWindow.ts` is read by exactly two queries — `getDiscoverFeed` and `getFriendsFeed` — and it is a filter on the read, never a delete. A post older than the window leaves the feeds and keeps everything else: it still counts toward `plateScore` and the category tallies, still appears in Saved, in your own activity, and in a restaurant's comments (all of which go through `getPosts`, which is deliberately unwindowed). Feed freshness and rating history are separate questions and must not be collapsed into one. Change the number in that module, not in the queries.
...
- **Counts that never display:** friend/follower counts, anywhere. `getFriends` returns rows without a total on purpose. Also no busyness or wait-time data — invented wait copy shipped once and was deliberately removed (PRODUCT.md), and there is no longer an exception: `ReservationPanel` + `src/lib/reservations.ts`, the disclaimed booking prototype that used to be one, are deleted. Nothing in the product displays a wait, a table or an availability.
